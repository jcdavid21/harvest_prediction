...
function setupAdjustmentControls() {
    ...
    if (yieldSlider) {
        // Change from 'input' to 'input' for real-time updates as slider moves
...
function updateAdjustedPrediction() {
    ...
    // Update charts with new adjusted values
...
function updateChartsWithAdjustments(baseValue, yieldAdjustment, productionRatio) {
    ...
    // Update Seasonal Comparison Chart
    if (window.seasonalComparisonChart) {
        const currentYearData = window.seasonalComparisonChart.data.datasets[0].data;
        const previousYearData = window.seasonalComparisonChart.data.datasets[1].data;
        
        // Get the base factor used for the original chart
        const maxCurrentYear = Math.max(...currentYearData);
        const baseFactor = window.originalBaseValue / maxCurrentYear;
        
        // Adjust current year data
        const adjustedCurrentYearData = currentYearData.map(val => 
            (val / baseFactor) * (baseValue * (1 + yieldAdjustment)) / window.originalBaseValue);
        
        // Adjust previous year data (keeping relative difference)
        const adjustedPreviousYearData = previousYearData.map(val => 
            (val / baseFactor) * (baseValue * (1 + yieldAdjustment)) / window.originalBaseValue);
        
        window.seasonalComparisonChart.data.datasets[0].data = adjustedCurrentYearData;
        window.seasonalComparisonChart.data.datasets[1].data = adjustedPreviousYearData;
        window.seasonalComparisonChart.update();
    }
}
...
function updateProvinceOptions(selectedRegion) {
    // This is a placeholder - in a real system you would filter provinces by region
    // For now, we'll just show all provinces
    const provinceSelect = document.getElementById('pred-province');
    provinceSelect.innerHTML = '<option value="" disabled selected>Select a province</option>';
    
    if (window.allProvinces) {
        window.allProvinces.forEach(province => {
            const option = document.createElement('option');
